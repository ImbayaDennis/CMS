...
const Sidebar = ({isOpen, setState}: Props) => {
  ...
  return (
    <div className={`bg-slate-800 border-r border-r-gray-900 ${width} p-4 flex flex-col items-center h-full transition-all`}>
      <button onClick={()=> setState(prev => (!prev))} className='text-2xl text-gray-500 hover:text-orange-400 py-2 transition-colors self-end'><HiMenu /></button>
...
const SidebarLink = ({link, icon, to, sidebarOpen, onClick}: SidebarLinkProps) =>{
    return <Link className='text-2xl text-gray-400 hover:text-orange-400 py-2  rounded-md transition-colors flex items-center flex-nowrap' aria-label={to} href={link} >{icon} <span className={`${sidebarOpen ? "opacity-100" : "opacity-0 hidden"} ml-6 text-base transition-opacity`}>{to}</span></Link>
}
