    ...
    .mutation(({ input, ctx }) => {
      return ctx?.prisma?.field.create({
        ...
        if(input.type === "string"){
          ctx?.prisma?.stringField.create({data: {fieldId: res.id, content: input.content}})
        }
        if(input.type === "textarea"){
          ctx?.prisma?.stringField.create({data: {fieldId: res.id, content: input.content}})
        }
        if(input.type === "number"){
          ctx?.prisma?.numberField.create({data: {fieldId: res.id, content: input.content}})
        }
        if(input.type === "boolean"){
          ctx?.prisma?.booleanField.create({data: {fieldId: res.id, content: input.content}})
        }
        if(input.type === "image"){
          ctx?.prisma?.imageField.create({data: {fieldId: res.id, content: input.content}})
        }
      })
    }),
});
